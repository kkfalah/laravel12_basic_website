<div class="lonyo-section-padding bg-heading position-relative sectionn">
    <div class="container">
        <div class="row">
            <div class="col-lg-5">
                <div class="lonyo-video-thumb">
                    <img 
                    src="{{ !empty($midSectionVideo->image) ? Storage::url($midSectionVideo->image) : asset('frontend/assets/images/video-thumb.png') }}"
                    alt="">
                    <a class="play-btn video-init" href="{{ $midSectionVideo->video_link }}">
                        <img src="{{ asset('frontend') }}/assets/images/v1/play-icon.svg" alt="">
                        <div class="waves wave-1"></div>
                        <div class="waves wave-2"></div>
                        <div class="waves wave-3"></div>
                    </a>
                </div>
            </div>
            <div class="col-lg-7 d-flex align-items-center">
                <div class="lonyo-default-content lonyo-video-section pl-50" data-aos="fade-up" data-aos-duration="500">
                    <h2>{{ $midSectionVideo->title }}</h2>
                    <p>{{ $midSectionVideo->description }}</p>
                    <div class="mt-50" data-aos="fade-up" data-aos-duration="700">
                        <a class="lonyo-default-btn video-btn" href="{{ $midSectionVideo->link }}">Download the app</a>
                    </div>
                </div>
            </div>
        </div>
        <div class="row">
            @foreach ($midSectionVideoBottom as $key => $item)
            <div class="col-xl-4 col-md-6">
                <div class="lonyo-process-wrap" data-aos="fade-up" data-aos-duration="500">
                    <div class="lonyo-process-number text-white">
                        <img src="{{ asset('frontend') }}/assets/images/v1/n{{ $item->id }}.svg" alt="">
                    </div>
                    <div class="lonyo-process-title">
                        <h4>{{ $item->title }}</h4>
                    </div>
                    <div class="lonyo-process-data">
                        <p>{{ $item->description }}</p>
                    </div>
                </div>
            </div>
            @endforeach
            
            <div class="border-bottom" data-aos="fade-up" data-aos-duration="500"></div>
        </div>
    </div>
</div>
